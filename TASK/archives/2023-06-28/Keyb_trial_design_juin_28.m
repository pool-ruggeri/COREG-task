...
for nblocks = 1:length(blocks)
    if strcmp(blocks{nblocks},'learn')
       ...
       % Text = ['Training for the task \n \n \n Press spacebar to continue'];
        ...
        %    Text = ['First part of the task \n \n \n Press spacebar to continue'];
            ...
            block = dysreg_block; % not  learning mode
            % draw text on screen
         %   Text = ['Second part of the task \n \n \n  Press spacebar to continue'];
        end
    end
    
    %initialisation of the outer radius depending on the block (smaller for
    %dysreg
    ...
    % we choose the text depending on the block
    Text = Text_b{block};
    % we put the text on screen
...
%             end
    ...
    if block == coreg_block
        data.posCoreg = [Xdim; Ydim];
        data.completeCoreg = trial_counter;
        data.touchedCoreg = touch_counter;
    else
        if block == dysreg_block
            data.posDysreg = [Xdim; Ydim];
            data.completeDysreg = trial_counter;
            data.touchedDysreg = touch_counter;
        end
    end
end
