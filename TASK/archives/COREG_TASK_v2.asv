...
Screen('TextFont', window, 'Arial'); Screen('TextSize', window, 24); Screen('TextColor',window,white);
...
Screen(window,'FillRect', gray);  % prepare grey screen for relaxing period
vbl = Screen('Flip',window); % flip the screen
vbl = Screen('Flip',window, vbl + 3 + 0.5*ifi); %present it for XX seconds

%% Before starting with the training period, we show the participants a short movie
Text = Text_movie;
Screen('TextFont', window, 'Arial'); Screen('TextSize', window, 40); Screen('TextColor',window,white);
DrawFormattedText(window, Text,'center','center');
...
if BIOPAC
    send_ns_trigger(trigger_movie,port)
end

movieFile = [moviepath filesep moviename];
...
for nblocks = 1:length(blocks)
    ...
    if strcmp(blocks{nblocks},'dysreg')
        ...
        % send trigger for onset relax period
        if BIOPAC
            send_ns_trigger(trigger_relax,port)
        end
        
        Screen(window,'FillRect', gray);  % prepare grey screen for relaxing period
        vbl = Screen('Flip',window); % flip the screen
        vbl = Screen('Flip',window, vbl + durationRelaxPeriodSecs + 0.5*ifi); %present it for XX seconds
    ...
    if BIOPAC
        if strcmp(blocks{nblocks},'learn')
            send_ns_trigger(trigger_training,port)
        elseif strcmp(blocks{nblocks},'coreg')
...
if BIOPAC
    send_ns_trigger(trigger_recovery,port)
end

Screen(window,'FillRect', gray);  % prepare grey screen for relaxing period
vbl = Screen('Flip',window); % flip the screen
vbl = Screen('Flip',window, vbl + durationRecoverySecs + 0.5*ifi); %present it for XX seconds
